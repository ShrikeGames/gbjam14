...
func _ready() -> void:
	...
	for i in range(0, Global.save_data["game"]["max_hp"] / 2):
...
func _process(_delta: float) -> void:
	if Global.save_data["game"]["hp"] <= 0:
		Global.save_data["game"]["event_name"] = "EVENT_HOSPITAL"
		Global.save_data["game"]["event_cost"] = 10
		Global.save_data["game"]["hp"] = Global.save_data["game"]["max_hp"]
		Global.save_data["game"]["day"] += 1
		get_tree().change_scene_to_file(home_scene)
	
func update():
	var target_hp: int = Global.save_data["game"]["max_hp"]
	var hearts: Array = self.get_children()
	hearts.reverse()
	for heart: Heart in hearts:
		var target_frame: int = clampi((target_hp - Global.save_data["game"]["hp"]), 0, 2)
		heart.play("%s" % [target_frame])
		target_hp -= 2
	
func _player_hurt():
	Global.save_data["game"]["hp"] = clampi(Global.save_data["game"]["hp"] - 1, 0, Global.save_data["game"]["max_hp"])
	update()

func _player_max_health_increase():
	var heart: Heart = Global.heart.instantiate()
	heart.position = Vector2(8 + ((Global.save_data["game"]["max_hp"] / 2) - 1) * 16, 8)
	self.add_child(heart)
	update()
